...
async function transmitBytes(bytes, str = "") {
    ...
    sendByte(start_symbol).then(_ => {
        // Send text
        console.log("Send text");
        sendBytes(bytes).then(_ => {
            // Send Stop
            console.log("Send stop");
            sendByte(stop_symbol).then(_ => {
                // Reset background
...
function sendBytes(bytes) {
    return recursiveByteTransmit(bytes);
}
function recursiveByteTransmit(bytes, i = 0) {
    if (i < bytes.length) {
        return sendByte(bytes[i]).then(_ => recursiveByteTransmit(bytes, i+1))
    } else {
        return Promise.resolve();
    }
}
function sendByte(byteStr) {
    return recursiveBitTransmit(byteStr)
}

function recursiveBitTransmit(chars, i = 0) {
    if (i < chars.length) {
        return sendBit(chars[i]).then(_ => {
            recursiveBitTransmit(chars, i + 1)
        });
    } else {
        return Promise.resolve();
    }
