        ...
        for line in output.split("\n"):
            if line.strip().startswith("SSID BSSID"):
                ...
                try:
                    ...
                    ap = AccessPoint(ssid, bssid, rssi, security)
                    results.append(ap)
        ...
        for num, line in enumerate(output.split("\n")):
            ...
            if line.startswith("SSID"):
                ...
                if bssid is not None:
                    ap = AccessPoint(ssid, bssid, quality_to_rssi(quality), security)
                    results.append(ap)
        return results
    ...
    def parse_output(self, output):
        ...
        for line in output.strip().split('\n'):
            try:
                ...
                continue
            access_point = AccessPoint(ssid, bssid, quality_to_rssi(quality), security)
    ...
    def parse_output(self, output):
        ...
        for num, line in enumerate(output.split("\n")):
            ...
            if line.startswith("Cell"):
                ...
                rssi = float(line.rsplit("=", 1)[1].split(' ')[0])
...
class TermuxWifiScanner(WifiScanner):
    ...
    def parse_output(self, output):
        ...
        return [
            AccessPoint(i['ssid'], i['bssid'], i['rssi'], '')
            for i in data
        ]
...
def get_scanner(device="", force_iwlist=False):
